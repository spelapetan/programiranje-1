...
(podčrtaj za nepomembne spremenljiveke
ker mamo function, ne rabimo list in match...)
...
let slice i k list =
  ...
  let (slice2, _) = divide (k - i) slice 1 in
  slice2
...
let rec insert x k list = function
  | [] -> [x]
...
let rec remove x list = function
  | [] -> []
