...
let zip_enum_tlrec list1 list2 = 
  let rec zipe_aux list1 list2 i aux =
    match list1, list2 with
    | [], [] -> reverse acc
    | _, [] | [], _ -> failwith "Different lengths of input lists."
    | x :: xs, y :: ys ->
      let element = (i, x, y) in
      zipe_aux xs xy (i + 1) (element :: acc)
    in
...
let unzip_tlrec list =
  let rec unzip_aux list acc1 acc2 =
    ...
    | (x, y) :: rep -> unzip_aux rep (x :: acc1) (y :: acc2)
    in
    unzip_aux list [] []
...
let rec exists f list = function
  | [] -> false
...
let rec first f default list = function
  | [] -> default
  | x :: xs -> if f x then x else f default xs
